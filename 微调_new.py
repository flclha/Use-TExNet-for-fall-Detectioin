...
from sklearn.metrics import roc_auc_score, roc_curve   # ★ 新增
import pandas as pd
import os
...
# ==================== 基本设置 ====================
device = 'cpu'
num_epochs_finetune = 80   # 不要太大，配合早停即可
batch_size = 128
...
# ★ 用于整体 ROC/AUC（把所有 fold 的测试样本汇总）
all_test_labels_global = []
all_test_scores_global = []   # 正类概率
...
for test_env in unique_envs:
    ...
    # ========== few-shot：从测试 subject 中抽少量样本加入训练 ==========
    if n_test_all > 3:

        n_support = max(1, int(n_test_all * support_ratio))
    ...
    with torch.no_grad():
        ...
        for inputs, labels, _ in test_loader:
            ...
            # ★ 预测得分（正类概率）
            probs = F.softmax(outputs, dim=1)[:, 1]   # 假设类别 1 为“正类”
            test_scores_all.extend(probs.cpu().numpy().tolist())
        ...
        # ★ 计算当前 fold 的 AUC
        test_auc = roc_auc_score(test_trues_all, test_scores_all)

        # ★ 将本 fold 的 test label & score 汇总到全局，以便最后画一条整体 ROC 曲线
        all_test_labels_global.extend(test_trues_all)
        all_test_scores_global.extend(test_scores_all)

    print(f"\n[Env {test_env}] 使用验证集 F2 最优模型在测试集上的最终结果："
          f"Test Acc: {test_acc:.4f}, Test Rec: {test_rec:.4f}, "
...
print("Mean AUC:", results_df['test_auc'].mean())   # ★ 平均 AUC
...
# =============== ★ 画整体 ROC 曲线（所有 fold 的测试样本汇总） ===============
if len(all_test_labels_global) > 0:
    fpr, tpr, _ = roc_curve(all_test_labels_global, all_test_scores_global)
    auc_global = roc_auc_score(all_test_labels_global, all_test_scores_global)

    plt.figure(figsize=(6, 6))
    plt.plot(fpr, tpr, label=f'ROC curve (AUC = {auc_global:.4f})')
    plt.plot([0, 1], [0, 1], 'k--', label='Random')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('Overall ROC Curve (LOSO + few-shot)')
